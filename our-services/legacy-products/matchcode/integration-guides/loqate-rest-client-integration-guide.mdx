...
1. The customer's server perform an AuthenticateUser call to the existing Matchcode360 REST Services by using the usual credentials ([Authentication and Fixed Duration Tokens](#authentication-and-fixed-duration-tokens)).
...
| ipAddress	| String	| (Optional) The IPv4 Address of the requesting machine. <br/><br/>Note that if an ipAddress is supplied to **authenticateUser**, then the same ipAddress must be provided with subsequent requests using this authentication. | 
| fixedduration	| String	| Mandatory for Client Side Integration. <br/><br/>This generates a "fixed duration" Token, to be used from the browser client side. Values: true. Other value than "true" would cause the Token not to be "fixed duration" | 
...
The JSON / XML to be sent to perform a Matchcode360 REST Request and the fields content as well as the responses from the REST WebService are the same regardless the origin of the calls (server or client). Thus, the user can query other articles to get that information ([Integration Fundamentals](http://localhost:3000/our-services/legacy-products/matchcode/general-information/integration-fundamentals)).